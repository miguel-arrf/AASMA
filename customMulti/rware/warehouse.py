...
class Warehouse(gym.Env):
    ...
    def __init__(
        ...
        # meu: self.action_space = MultiAgentActionSpace([spaces.Discrete(5) for _ in range(self.n_agents)])
    ...
    def _make_obs(self, agent):
        ...
        for goal in self.goals:
            goal_x = goal[0]
            goal_y = goal[1]
            # caso esteja na mesma posição
            if goal_x == agent.x and goal_y == agent.y:
                obs["goal"] = (goal_x, goal_y)
            # caso o agente esteja em cima
            elif goal_x == agent.x and goal_y == agent.y+1:
                obs["goal"] = (goal_x, goal_y)
            # caso esteja Á direita
            elif goal_x == agent.x-1 and goal_y == agent.y:
                obs["goal"] = (goal_x, goal_y)
            # À esquerda
            elif goal_x == agent.x+1 and goal_y == agent.y:
                obs["goal"] = (goal_x, goal_y)
    ...
    def step(
        ...
        for y, x in self.goals:
            ...
            oldShelf = shelf
            self.request_queue.remove(oldShelf)

            otherShelfs = []
            for shelf in self.shelfs:
                if shelf not in self.request_queue:
                    otherShelfs.append(shelf)

            self.request_queue.append(np.random.choice(otherShelfs, size=1))
        ...
        if all(dones):
            print("indeed")
        '''
        # return new_obs, rewards[0], all(dones), info
