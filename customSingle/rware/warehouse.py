...
class Warehouse(gym.Env):
    ...
    def __init__(
        ...
        self.give_reward_help = give_reward_help
        ...
        # self.action_space = MultiAgentActionSpace([spaces.Discrete(5) for _ in range(self.n_agents)])[0]
    ...
    def _make_layout_from_params(self, shelf_columns, shelf_rows, column_height):
        ...
        # Sítio onde é feita a decisão para criar os goal positions
        self.goals = [
    ...
    def _use_fast_obs(self):
        ...
        self.observation_space = spaces.Tuple(tuple(ma_spaces))[0]
    ...
    def _make_obs(self, agent):
        ...
        if self.fast_obs:
            ...
            obs = _VectorWriter(self.observation_space.shape[0])
        ...
        for goal in self.goals:
            goal_x = goal[0]
            goal_y = goal[1]
            if goal_x == agent.x and goal_y == agent.y:
                obs["goal"] = (goal_x, goal_y)
        ...
        for i, id_ in enumerate(shelfs):
            obs["sensors"][i]["isWall"] = []

            if i in [0, 1, 2]:
                if agent_y == 0:
                    obs["sensors"][i]["isWall"].append("TOP")

            if i in [6, 7, 8]:
                if agent_y == self.height - 1:
                    obs["sensors"][i]["isWall"].append("BOTTOM")

            if i in [0, 3, 6]:
                if agent_x == 0:
                    obs["sensors"][i]["isWall"].append("LEFT")

            if i in [2, 5, 8]:
                if agent_x == self.width - 1:
                    obs["sensors"][i]["isWall"].append("RIGHT")
    ...
    def reset(self):
        ...
        return [self._make_obs(agent) for agent in self.agents][0]
        # for s in self.shelfs:
    ...
    def step(
        ...
        for y, x in self.goals:
            ...
            if self.reward_type == RewardType.GLOBAL:
                ...
                rewards[agent_id - 1] += 100
            elif self.reward_type == RewardType.TWO_STAGE:
                agent_id = self.grid[_LAYER_AGENTS, x, y]
                self.agents[agent_id - 1].has_delivered = True
                rewards[agent_id - 1] += 50

        def hamming_distance(agent_x, agent_y, shelf_x, shelf_y):
            return abs(agent_x - shelf_x) + abs(agent_y - shelf_y)

        # print("x: {}, y: {}".format(self.request_queue[0].x, self.request_queue[0].y))
        # print("agent: x: {}, y: {}".format(self.agents[0].x, self.agents[0].y))

        agent_x = self.agents[0].x
        agent_y = self.agents[0].y
        agent_old_x = self.agents[0].prev_x
        agent_old_y = self.agents[0].prev_y

        request_queue_x = self.request_queue[0].x
        request_queue_y = self.request_queue[0].y

        current_distance = hamming_distance(agent_x, agent_y, request_queue_x, request_queue_y)
        old_distance = hamming_distance(agent_old_x, agent_old_y, request_queue_x, request_queue_y)

        if self.give_reward_help:
            if not self.agents[0].carrying_shelf:
                if current_distance < old_distance:
                    rewards += 0.2
                else:
                    rewards -= 0.1

            if self.agents[0].carrying_shelf:
                for goal_x, goal_y in self.goals:
                    current_distance_goal = hamming_distance(agent_x, agent_y, goal_x, goal_y)
                    old_distance_goal = hamming_distance(agent_old_x, agent_old_y, goal_x, goal_y)

                    if current_distance_goal < old_distance_goal:
                        rewards += 0.2
                    else:
                        rewards -= 0.1
        ...
        for i in range(self.n_agents):
            done_agents['agent_{}'.format(i)] = dones[i]
            reward_agents['agent_{}'.format(i)] = list(rewards)[i]
            obs_agents['agent_{}'.format(i)] = new_obs[i]
        ...
        i_am_done = False
        if all(dones) or shelf_delivered:
            i_am_done = True

        return new_obs[0], rewards[0], i_am_done, info
